...
func SubmitBidFeedbackHandler(w http.ResponseWriter, r *http.Request) {
	...
	var bid models.Bid
	if err := db.QueryRow(query, bidFeedback, bidID, username).Scan(&bid.ID, &bid.TenderID, &bid.Username, &bid.Name, &bid.Description, &bid.Decision, &bid.Feedback, &bid.CreatedAt); err != nil {
		log.Error(err.Error())
		http.Error(w, "Something went wrong", http.StatusInternalServerError)
		return
	}
...
func RollbackBidHandler(w http.ResponseWriter, r *http.Request) {
	...
	var bid models.Bid
	if err := db.QueryRow(query, bidID, versionInt).Scan(&bid.ID, &bid.TenderID, &bid.Username, &bid.Name, &bid.Description, &bid.Decision, &bid.Feedback, &bid.Version, &bid.CreatedAt, &bid.UpdatedAt); err != nil {
		log.Error(err.Error())
	...
	if err := db.QueryRow(query, bid.Name, bid.Description, bid.Decision, bid.Feedback, bidID, username).Scan(&bid.ID, &bid.TenderID, &bid.Username, &bid.Name, &bid.Description, &bid.Decision, &bid.Feedback, &bid.Version, &bid.CreatedAt, &bid.UpdatedAt); err != nil {
		log.Error(err.Error())
		http.Error(w, "Something went wrong", http.StatusInternalServerError)
		return
	}
